...
def minimum_assignment(cost_matrix):
    #rumus
...
def main():
    st.title("Persoalan Penugasan Minimum")
    #input pekerja & tugas
    ...
    st.write('Jika pekerja tidak bisa melakukan tugas tertentu isi dengan biaya terbesar')
    for bp in range(p):
        for bt in range(t):
            #input biaya
    ...
    if st.button("Hitung"):
        assignments, total_cost = minimum_assignment(cost_matrix)
        st.write("Hasil Penugasan Minimum:")
